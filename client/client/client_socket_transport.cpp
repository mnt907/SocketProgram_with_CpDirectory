...
namespace
{
	...
	bool SendFileData(const std::string& src_path, const SOCKET& socket, Header& header)
	{
		errno_t fp_err;
		FILE* src_fp = NULL;
		fp_err = fopen_s(&src_fp, src_path.c_str(), "rb");
		if (fp_err != 0)
		{
		...
		for (int i = 0; i < COPY_NUMBER; i++)
		{
			...
			if (sendlen == -1)
			{
				std::cout << "can't send file_data" << errno << std::endl;
				return false;
			}
...
int main()
{
	...
	while (true) {
		...
		char src_path[PACKET_SIZE] = "C:/Users/kyoud/Desktop/a";
		char dst_path[PACKET_SIZE] = "C:/Users/kyoud/Desktop/b";
