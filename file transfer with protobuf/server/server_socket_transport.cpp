...
namespace
{
...
#endif // _WIN32
    bool SendToClient(const int socket, char* send_data, int data_size)
    {
...
#else
        int sendlen = send(socket, (char*)&send_data_size, sizeof(int), MSG_NOSIGNAL);
#endif //_WIN32
        if (sendlen == -1)
        {
    ...
    bool RecvFileData(const int socket, const Header& header)
    {
...
#ifdef _WIN32
        int write_fp_err = fopen_s(&dst_fp, header.m_file_path().c_str(), "wb");
        if (write_fp_err != 0)
        {
            ...
            if (write_fp_err == 13)
            {
                ...
                error_header.set_m_type(Header::TYPE::Header_TYPE_ERROR_SIG);
                error_header.set_m_file_path(error_msg);

                int str_size = error_header.ByteSizeLong() ;
                char* serial_str = new char[str_size];
                error_header.SerializeToArray(serial_str, str_size);
                if (SendToClient(socket, serial_str, str_size) == false)
                {
                    delete[] serial_str;
                    return false;
                }
        ...
        int64_t sum_recv_len = header.m_length();
    ...
    bool CopyDirectory(const int socket, const Header& header)
    {
        ...
        if (header.m_is_dir() == true)
        {
            std::cout << "make directory" << std::endl;
#ifdef _WIN32
            int result = _mkdir(header.m_file_path().c_str());
#else
            ...
            {
                if (errno == EEXIST)
                    std::cout << "already exist " << header.m_file_path() << std::endl;
                else if (errno == ENOTDIR)
                {
                    std::cout << header.m_file_path() << " is not directory" << std::endl;
                    return false;
                }
                else
                {
                    std::cout << "mkdir error :  " << errno << "  " << header.m_file_path() << std::endl;
                    return false;
                }
            }
            std::cout << "success mkdir " << header.m_file_path() << std::endl;
        }
        else
        {
        ...
        , std::condition_variable* cpy_cv, std::condition_variable* recv_cv, std::mutex* m)
    {
        ...
        while (end_signal == false)
        {
            ...
            if (header.m_type() == Header::TYPE::Header_TYPE_CONNECT)
            {
                if (IsExistDirectory(header.m_file_path()))
                {
                ...
                else
                {
                    ...
                    header.set_m_type(Header::TYPE::Header_TYPE_NOT_EXIST_DIR);
                }
                
                std::cout << "dst_path : " << header.m_file_path() << std::endl;
            ...
            else if (header.m_type() == Header::TYPE::Header_TYPE_CHECK_DIR)
            {
                if (CopyDirectory(socket, header) == false)
                {
                    ...
                    header.set_m_type(Header::TYPE::Header_TYPE_ERROR_SIG);
            ...
            while (end_signal == false)
            {
                ...
                for (int i = 0; i < fd_max + 1; ++i)
                {
                    ...
                    if (FD_ISSET(socket, &cpy_read_set))
                    {
                        ...
                        const std::string converted_dst_path = Utf8ToMultiByte(header.m_file_path());
